...
from .forms import MembershipForm,AdvertisementForm,MembershipForm1,AdvertisementForm1
import json
from .models import Membership,Advertisement,CollegeMembership,CollegeAdvertisement
...
@csrf_exempt
def membership_form_view1(request):
    if request.method == 'POST':
        try:
...
@csrf_exempt
def advertisement_form_view(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            email = data.get('email')

            if Advertisement.objects.filter(email=email).exists():
                return JsonResponse({
                    'status': 'error',
...
@csrf_exempt
def advertisement_form_view1(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            email = data.get('email')
...
# from django.http import JsonResponse
# from django.views.decorators.csrf import csrf_exempt
# from .models import CollegeAdvertisement, UniversityInCharge
# from .forms import AdvertisementForm1
